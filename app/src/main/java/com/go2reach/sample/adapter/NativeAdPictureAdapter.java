...
public class NativeAdPictureAdapter extends BaseAdapter {
	List<Item> items;
	LayoutInflater inflater;
	public NativeAdPictureAdapter(Context context, List<Item> items){
		this.items = items;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (itemView == null){
			if (item.getType() == Item.TYPE_PRODUCT){
				itemView = inflater.inflate(R.layout.list_item_product_pic, parent, false);
			} else {
				itemView = inflater.inflate(R.layout.list_item_ad_pic, parent, false);
			}
		}
		if (item.getType() == Item.TYPE_PRODUCT){
			...
			item.getAsAdItem().bind(itemView,
					new String[]{IAdItem.TITLE, IAdItem.IMAGE},
					new int[]{R.id.tvListItemTitle, R.id.ivListItemImage}
			);
		}
		return itemView;
	}
